document.addEventListener('DOMContentLoaded', async function () {
    ...
    function createApiItemElement(itemName, item, isLastItem) {
        ...
        itemDiv.dataset.status = item.status || 'online';
        ...
        statusIndicator.className = `w-2 h-2 rounded-full ${item.status === 'online' ? 'bg-green-500' : 'bg-red-500'}`;
        statusIndicator.title = item.status === 'online' ? 'Online' : 'Offline';
        ...
        button.className = `px-4 py-2 text-sm font-medium text-white w-13 h-9 flex items-center justify-center transition duration-300 flex-shrink-0 rounded ${
            item.status === 'online' 
        ...
        button.dataset.apiStatus = item.status || 'online';
        button.textContent = 'GET';
        
        // Disable button if API is offline
    ...
    function setupApiButtonHandlers(gtw) {
        document.addEventListener('click', event => {
            if (event.target.classList.contains('get-api-btn')) {
                const { apiPath, apiName, apiDesc, apiStatus } = event.target.dataset;

                // Check if API is offline
                if (apiStatus !== 'online') {
                    alert('This API is currently offline. Please try again later.');
                    return;
                }
                ...
                openApiModal(apiName, apiPath, apiDesc, 'GET', apiStatus);
            }
        });
    }
                ...
                if (nextElement && nextElement.classList.contains('row')) {
                    ...
                    visibleItems.forEach((item, index) => {
                        ...
                        if (button) {
                            ...
                            button.dataset.apiStatus = item.element.querySelector('.get-api-btn')?.dataset.apiStatus || 'online';
                        }
    ...
    function openApiModal(name, endpoint, description, method, status) {
        // Check if API is offline
        if (status !== 'online') {
            alert('This API is currently offline. Please try again later.');
            return;
