(async () => {
    ...
    app.use((req, res, next) => {
        if (set.maintenance && req.path !== '/maintenance' && req.path !== '/set') {
                ...
                try {
                    ...
                    if (module && module.run && typeof module.run === 'function') {
                        ...
                        endpointObj[module.name || endpointName] = {
                            ...
                            status: apiStatus // Add status to endpoint data
                        };
                        ...
                        if (apiStatus === 'online') {
                            ...
                            logger.warn(`${chalk.yellow(endpointPath)} ${chalk.dim('(')}${chalk.cyan(category)}${chalk.dim(')')} ${chalk.red('[OFFLINE]')}`);
                        }
                    }
                } catch (error) {
    ...
    app.listen(PORT, () => {
        ...
        if (set.maintenance) {
            logger.warn(`Maintenance mode: ${chalk.yellow('ENABLED')}`);
        }
